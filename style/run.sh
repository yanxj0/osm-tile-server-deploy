...
if [ "$1" == "import" ]; then
    ...
    if [ ! -f /etc/postgresql/14/main/conf.d/postgresql.custom.conf ]; then
    ...
    if [ ! -f /data/region.osm.pbf ] && [ -z "${DOWNLOAD_PBF:-}" ]; then
        echo "WARNING: No import file at /data/region.osm.pbf, so importing guangdong as example..."
        DOWNLOAD_PBF="https://download.geofabrik.de/asia/guangdong-latest.osm.pbf"
        DOWNLOAD_POLY="https://download.geofabrik.de/asia/guangdong.poly"
    fi
